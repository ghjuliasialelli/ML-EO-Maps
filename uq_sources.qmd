...
The appropriate choice depends on what will be done with the uncertainty. If the goal is to mask out unreliable regions, even a rough uncertainty proxy may suffice. If the goal is to produce calibrated prediction intervals for carbon accounting, the requirements are stringent and the cost is correspondingly higher. If the goal is to report aggregate uncertainty over administrative units, the dominant concern is spatial error correlation (Section [Spatial Structure of Uncertainty](uq_spatial.qmd)), and no pixel-level UQ method alone will suffice. Users should articulate the intended use of uncertainty before choosing a method, not after. @fig-uq-flowchart summarizes the decision logic discussed in the sections that follow.

![Decision logic for choosing an uncertainty quantification approach, starting from the intended use of the uncertainty: filtering unreliable pixels, reporting calibrated prediction intervals, aggregating uncertainty over regions, or estimating uncertainty of temporal change.](images/uq_flowchart.png){#fig-uq-flowchart}
