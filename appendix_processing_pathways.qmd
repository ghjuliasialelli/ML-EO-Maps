...
For Sentinel-2, the differences between CDSE and GEE are summarized in @tbl-s2-pathways. The atmospheric correction is identical between the two platforms, so observed differences in surface reflectance might arise from baseline currency or radiometric offset handling.

| **Aspect** | **CDSE (native)** | **GEE** |
| --- | --- | --- |
| Radiometric offset (post PB 04.00) | $+1000$ DN baked in | Subtracted in `HARMONIZED` collections; passed through unchanged in non-harmonized collections |
...
: Processing differences between native Sentinel-2 products on CDSE and the corresponding GEE collections. DN: digital number. {#tbl-s2-pathways}
...
| Radiometric terrain correction | × | Available via `gamma0_terrain` + orthorectification | × | × |
